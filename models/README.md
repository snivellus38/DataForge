...
Two byte-level BDH language models — English→French and English→Portuguese — plus the failed
merge of the two, the notebook that trained them, and the telemetry the training run recorded
about itself.
...
**These checkpoints are prior work.** They were trained by one member of this team at an earlier
Pathway hackathon, before DataForge 2026, and are reused here with disclosure (required by the
problem statement, line 178). What is *new* for this submission is everything derived from them:
[`research/bdh_big.py`](../research/bdh_big.py), every `research/export_*.py`, the analysis, the
three pages, and the tests.
...
retraining. They are disclosed everywhere these models appear on screen:
...
It generates real French: `Le Parlement europeen a vote contre cette resolution`, `Merci
beaucoup.`
...
| `telemetry/graph/gstar_head*.json` | the prior run's `G*` edge lists | cross-check against `research/export_field.py` |
| `telemetry/monosemanticity/precomputed.json` | the prior run's concept selectivity — **uncontrolled**, 200 neurons at 1.0 | kept as the contrast for [`research/export_concepts.py`](../research/export_concepts.py), which redoes it against a permutation null |
...
| `prior-run-telemetry.tar.gz` | 33 MB | the prior run's raw per-token dumps and hero-token frames |
...
- **The merge fails, and that is not a refutation of the paper.** §7.1's recipe assumes two models
  forked from a common base; these two were trained independently from scratch, so
  [`research/merge_replicate.py`](../research/merge_replicate.py) tests a harder case than the
  paper claims. Concatenating neurons is structurally well defined but a neuron's *meaning* is
